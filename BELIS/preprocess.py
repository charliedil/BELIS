...
def tokenize(path):
    ...
    for file in os.listdir(path):
        ...
        bert_offsets = tokenizer(spacy_tok_sents, padding=True, truncation=True, max_length=512, return_tensors="pt",
                                 is_split_into_words=True, return_offsets_mapping=True).offset_mapping

        bert_tokens = tokenizer(spacy_tok_sents, padding=True, truncation=True, max_length=512,
                                return_tensors="pt", is_split_into_words=True)  # TODO try return_offsets_mapping to remove above line
