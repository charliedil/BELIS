...
def nearest_centroid_classifier(docs, cv): ##ADJUST NUMBERS TO 10 WHEN WITH OTHER, 9 WITHOUT
    X = []
    y = []

    for doc in docs:
        doc_embeddings, doc_labels = get_word_embeddings_and_labels(doc) ##change depending on the level of embeddings
    ...
    # print(scores)
    if cv:
        stratkfold = StratifiedKFold(shuffle=True, n_splits=2)
        iter = 1
        ...
        y = []
        for doc in docs:
            doc_embeddings, doc_labels = get_word_embeddings_and_labels(
                doc)  ##change depending on the level of embeddings
            for i in range(len(doc_embeddings)):
                X.append(doc_embeddings[i])
                y.append(doc_labels[i])
        ...
        label_dist = {}
        label_mapping = {}
        labels = {"Drug": 0, "Reason": 0, "Route": 0, "Form": 0, "ADE": 0, "Duration": 0, "Strength": 0, "Dosage": 0,
                  "Frequency": 0, "Other": 0}
        for i in range(len(kmeans.labels_)):
            labels[y[i]] += 1
            if kmeans.labels_[i] not in label_dist:
                label_dist[kmeans.labels_[i]] = {}
            if y[i] not in label_dist[kmeans.labels_[i]]:
                label_dist[kmeans.labels_[i]][y[i]] = 0
            label_dist[kmeans.labels_[i]][y[i]] += 1
        labels_sorted = dict(sorted(labels.items(), key=lambda item: item[1], reverse=True))
        print(labels_sorted)
        for label in labels_sorted:
            max_centroid = -1
        ...
        return kmeans.cluster_centers, label_mapping
